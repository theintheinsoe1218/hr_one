<template>
  <div class="overtime-page">
    ...
    <div class="d-flex flex-column flex-sm-row align-sm-center justify-sm-space-between mb-6 gap-4">
      <div>
        <h1 class="text-h5 text-md-h4 font-weight-bold text-grey-darken-4 mb-1">Overtime Management</h1>
        <div class="text-caption text-md-subtitle-1 text-grey-darken-1">Track and approve overtime requests</div>
      </div>
      <v-btn 
        color="primary" 
    ...
    <v-card class="rounded-xl elevation-2 pa-6">
      <div class="d-flex flex-column flex-md-row align-md-center justify-space-between mb-6 gap-4">
        ...
        <v-text-field
          ...
          hide-details
          color="primary"
          class="bg-grey-lighten-4 rounded-lg search-field"
      ...
      >
        <template v-slot:item.employee="{ item }">
          <div class="d-flex align-center py-3">
            <v-avatar color="primary-lighten-5" size="36" class="mr-3 text-primary font-weight-bold border-light">
              {{ getInitials(item.employee) }}
            </v-avatar>
            <span class="font-weight-medium text-on-surface">{{ item.employee }}</span>
          </div>
        </template>
    ...
    <v-dialog v-model="dialog" max-width="500px">
      <v-card class="rounded-xl">
        ...
        <v-card-text class="pa-6 pt-0">
          ...
          <v-row class="mb-5">
            <v-col cols="6">
              <label class="text-subtitle-2 font-weight-bold mb-1 d-block">Hours</label>
              <v-text-field
                placeholder="0"
                variant="outlined"
                hide-details
            ...
            <v-col cols="6">
              <label class="text-subtitle-2 font-weight-bold mb-1 d-block">Rate Multiplier</label>
              <v-select
          ...
          <div>
            <label class="text-subtitle-2 font-weight-bold mb-1 d-block">Reason</label>
            <v-textarea
        ...
        <v-card-actions class="pa-6 pt-0">
          ...
          <v-btn color="primary" rounded="lg" class="px-6 text-none font-weight-bold ml-3 shadow-primary" @click="dialog = false">Submit</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
...
const logs = ref([
  { id: 1, employee: 'James Wilson', date: 'Jan 17, 2025', hours: 3, rate: '1.5x', reason: 'Quarter-end financial reporting', status: 'Approved' },
  { id: 2, employee: 'Sarah Johnson', date: 'Jan 18, 2025', hours: 4, rate: '1.5x', reason: 'Product launch deadline', status: 'Pending' },
  { id: 3, employee: 'Michael Chen', date: 'Jan 15, 2025', hours: 2, rate: '2.0x', reason: 'System emergency maintenance', status: 'Rejected' },
])
...
const otStats = computed(() => {
  ...
  const totalHours = logs.value.filter(l => l.status === 'Approved').reduce((acc, curr) => acc + curr.hours, 0)
  
  return [
    { label: 'Pending Requests', value: pending, color: 'warning', bgColor: 'warning-lighten-5' },
    { label: 'Approved', value: approved, color: 'success', bgColor: 'success-lighten-5' },
    { label: 'Total OT Hours', value: `${totalHours}h`, color: 'primary', bgColor: 'primary-lighten-5' },
  ]
})
