<template>
  <div class="recruitment-page">
    <!-- Header -->
    <div
      class="d-flex flex-column flex-sm-row align-sm-center justify-sm-space-between mb-8 gap-4"
    >
      <div>
    ...
    <v-row class="mb-8">
      <v-col
        v-for="stat in recruitmentStats"
        :key="stat.label"
        cols="12"
        sm="6"
        md="3"
        ...
        <v-card class="rounded-xl pa-5 elevation-1 border-light">
          <div class="d-flex align-center">
            ...
            <div>
              ...
              <div class="text-h4 font-weight-black text-on-surface">
                {{ stat.value }}
              </div>
            </div>
          </div>
    ...
    >
      <div class="d-flex align-center gap-4">
        <div
          class="tab-container bg-surface pa-1 rounded-lg d-flex border-light"
        >
          <v-btn
            ...
            :class="{
              'active-tab elevation-1': activeTab === tab,
              'text-grey-darken-1': activeTab !== tab,
            }"
            rounded="lg"
        ...
        <div
          ...
          class="bg-surface pa-1 rounded-lg d-flex border-light"
        >
          <v-btn
            icon="mdi-view-list"
            variant="flat"
    ...
    <!-- Job Postings Table -->
    <v-card
      v-if="activeTab === 'Job Postings'"
      class="rounded-xl elevation-2 overflow-hidden border-light"
    >
      <v-data-table
        :headers="jobHeaders"
        :items="jobs"
    ...
    <template v-else>
      ...
      <div v-else class="kanban-board-wrapper mt-4 custom-scrollbar pb-6">
        <div class="kanban-containers d-flex gap-4">
          ...
          >
            <div
              class="d-flex align-center justify-space-between mb-4 px-2 pt-1 border-b-light pb-2"
            >
              <div class="d-flex align-center">
                <div :class="`stage-dot bg-${getStageColor(stage)} mr-2`"></div>
                <span
                  class="text-subtitle-2 font-weight-black text-on-surface"
                  >{{ stage }}</span
                >
              </div>
              <v-chip
                size="x-small"
                color="surface"
                class="font-weight-bold text-on-surface elevation-0 border-light px-2"
              >
            ...
            <div class="kanban-items d-flex flex-column gap-3 h-100">
              ...
              >
                <div class="d-flex align-start justify-space-between mb-2">
                  <v-avatar
                    :color="`${getStageColor(stage)}-lighten-5`"
                    size="32"
                    class="text-caption font-weight-bold"
                    :class="`text-${getStageColor(stage)}`"
                  >
                ...
                <div
                  class="text-subtitle-2 font-weight-bold text-on-surface mb-1"
                >
                  {{ applicant.name }}
                </div>
                <div
                  class="text-caption text-on-surface opacity-70 mb-3 line-clamp-1"
                >
                  {{ applicant.position }}
                </div>
                ...
                <div class="d-flex align-center justify-space-between">
                  <div
                    class="d-flex align-center text-caption text-on-surface opacity-60"
                  >
              ...
              <!-- Drop area filler -->
              <div
                v-if="getApplicantsByStage(stage).length === 0"
                class="flex-grow-1 d-flex flex-column justify-center align-center py-10 opacity-30 border-dashed rounded-xl"
              >
      ...
      <v-card class="rounded-xl overflow-hidden">
        ...
        >
          ...
          <v-btn
            ...
            variant="text"
            size="small"
            @click="addApplicantDialog = false"
          ></v-btn>
        </v-card-title>

        <v-card-text
          class="pa-6 pt-0 custom-scrollbar"
        ...
        >
          ...
          <v-row class="mb-4">
            <v-col cols="4">
              <label
                class="text-subtitle-2 font-weight-bold mb-1 d-block text-truncate"
            ...
            <v-col cols="6">
              ...
              <v-menu v-model="closingDateMenu" :close-on-content-click="false">
                ...
                <v-date-picker
                  @update:model-value="
                    (val) => {
                      jobForm.closingDate = val.toISOString().split('T')[0];
...
.bg-indigo-lighten-5-flat {
  background-color: #f5f7ff !important;
}
.bg-deep-purple-lighten-5-flat {
  background-color: #f8f5ff !important;
}
.bg-amber-lighten-5-flat {
  background-color: #fffaf0 !important;
}
.bg-success-lighten-5-flat {
