  ...
  <div class="payroll-page">
    <div v-if="!payslipDialog">
      ...
      >
        <div>
          <h1
            class="text-h5 text-md-h4 font-weight-bold text-grey-darken-4 mb-1"
          >
            Payroll
          </h1>
      ...
      <v-row class="mb-6">
        <v-col v-for="stat in payrollStats" :key="stat.label" cols="12" sm="4">
          <v-card
            class="rounded-xl pa-4 elevation-0 border-light overflow-hidden"
          >
        ...
        >
          <template v-slot:item.employee="{ item }">
            <div class="d-flex align-center py-3">
              <v-avatar
                ...
                size="36"
                class="mr-3 text-primary font-weight-bold"
              >
          ...
          <template v-slot:item.baseSalary="{ item }">
            <span class="font-weight-bold"
              >${{ formatNumber(item.baseSalary) }}</span
            >
          </template>

          <template v-slot:item.overtime="{ item }">
            <span class="text-success font-weight-medium"
              >+${{ formatNumber(item.overtime) }}</span
            >
          </template>

          <template v-slot:item.bonuses="{ item }">
            <span class="text-success font-weight-medium"
              >+${{ formatNumber(item.bonuses) }}</span
            >
          </template>

          <template v-slot:item.deductions="{ item }">
            <span class="text-error font-weight-medium"
              >-${{ formatNumber(item.deductions) }}</span
            >
          </template>

          <template v-slot:item.netSalary="{ item }">
            <span class="font-weight-bold"
              >${{ formatNumber(item.netSalary) }}</span
            >
          </template>
          ...
          <template v-slot:item.actions="{ item }">
            <div class="d-flex align-center gap-2">
              ...
              <v-btn
                ...
                variant="outlined"
                size="small"
                class="rounded-lg border-grey text-grey-darken-1"
    ...
    <div v-else class="payslip-container">
      ...
      <div class="d-flex justify-center">
        <v-card
          id="payslip-content"
          class="rounded-xl pa-8 pa-md-12 elevation-0 border-light"
          width="100%"
          max-width="900px"
        >
          <!-- Payslip Header -->
          <div class="d-flex justify-space-between align-start mb-14">
            <div class="d-flex align-center">
              <v-card
                ...
                width="56"
                height="56"
                class="d-flex align-center justify-center mr-4 elevation-0"
              >
            ...
            <div class="text-right">
              <div
                class="text-h5 font-weight-black text-primary mb-1 letter-spacing-1"
              >
                PAYSLIP
              </div>
              <div
                class="text-subtitle-1 text-grey-darken-1 font-weight-medium"
              >
                {{ selectedMonth }}
              </div>
            </div>
          ...
          <v-row class="mb-12">
            <v-col cols="12" md="6">
              <div
                class="text-subtitle-1 font-weight-bold text-grey-darken-3 border-b-light pb-2 mb-6"
              >
                Employee Details
              </div>
              <div
                v-for="(val, label) in employeeDetails"
                :key="label"
            ...
            <v-col cols="12" md="6">
              ...
              <div
                v-for="(val, label) in paymentDetails"
                :key="label"
                class="d-flex justify-space-between mb-3"
              >
                <span class="text-body-2 text-grey-darken-1">{{ label }}:</span>
                <span
                  ...
                  :class="
                    label === 'Status' ? 'text-warning' : 'text-grey-darken-4'
                  "
                >
                  {{ val }}
                </span>
          ...
          <v-row class="mb-14">
            <v-col cols="12" md="6">
              <v-card
                flat
                class="rounded-xl border-success overflow-hidden h-100 bg-white"
              >
                <div
                  class="bg-success-lighten-5 text-success font-weight-bold text-subtitle-1 px-5 py-3 d-flex align-center"
                >
                  Earnings
                </div>
                <div class="pa-6">
                  <div
                    v-for="(val, label) in earningsData"
                    :key="label"
            ...
            <v-col cols="12" md="6">
              <v-card
                flat
                class="rounded-xl border-error overflow-hidden h-100 bg-white"
              >
                <div
                  class="bg-error-lighten-5 text-error font-weight-bold text-subtitle-1 px-5 py-3 d-flex align-center"
                >
                  Deductions
                </div>
                <div class="pa-6">
                  <div
                    v-for="(val, label) in deductionsData"
                    :key="label"
                    class="d-flex justify-space-between mb-4"
                  >
                    <span class="text-body-1 text-grey-darken-1">{{
                  ...
                  <div class="d-flex justify-space-between pt-1">
                    ...
                    <span class="text-h6 font-weight-black text-error"
                      >${{ formatNumber(totalDeductions) }}</span
                    >
                  </div>
                </div>
              </v-card>
          ...
          <v-card
            flat
            color="#8b5cf6"
            class="rounded-xl pa-8 pa-md-10 text-white d-flex flex-column flex-sm-row justify-space-between align-sm-center mb-10 overflow-hidden"
          >
            <div class="relative-z">
              <div class="text-subtitle-1 font-weight-medium opacity-80 mb-2">
                Net Pay
              </div>
          ...
          <div
            class="d-flex flex-column flex-sm-row justify-space-between align-end"
          >
            <div>
              <div
                class="text-caption text-grey-darken-1 font-weight-medium mb-1"
              >
                Generated on {{ currentFormattedDate }}
              </div>
      ...
      <v-card class="rounded-xl pa-2">
        ...
        <v-card-text class="pa-4 pt-0">
          ...
          <v-row class="mb-4">
            <v-col cols="6">
              <label
                class="text-subtitle-2 font-weight-bold mb-1 d-block text-on-surface"
                >Pay Period</label
              >
              <v-menu v-model="periodMenu" :close-on-content-click="false">
                ...
                <v-date-picker
                  @update:model-value="
                    (val) => {
                      payrollForm.period = val.toISOString().substring(0, 7);
                      periodMenu = false;
                    }
                  "
                  color="primary"
          ...
          <v-card
            ...
            class="rounded-xl pa-4 d-flex justify-space-between align-center"
          >
            <div>
        ...
        <v-card-actions class="pa-4 pt-0">
          ...
          <v-btn
            ...
            @click="createSinglePayroll"
            >Create Payroll</v-btn
          >
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="autoGenDialog" max-width="500px">
      <v-card class="rounded-xl">
        ...
        <v-card-text class="pa-6 pt-0">
          <v-alert
            color="indigo-lighten-5"
            class="rounded-xl text-primary border-none mb-6"
            variant="flat"
          >
            ...
            <div class="text-caption">
              Automatically calculates base salary, overtime from approved
              records, and taxes.
            </div>
          </v-alert>
          ...
          <div class="d-flex justify-space-between align-center mb-4">
            <span class="text-subtitle-2 font-weight-bold"
              >Select Employees ({{ selectedEmps.length }} selected)</span
            >
            <v-btn
              variant="text"
              size="small"
            ...
            >
              {{
                selectedEmps.length === autoGenList.length
                  ? "Deselect All"
                  : "Select All"
              }}
            </v-btn>
          ...
          <div class="employee-scroll-list custom-scrollbar pr-1">
            <div
              v-for="emp in autoGenList"
              :key="emp.name"
              class="d-flex align-center py-3 border-b-light"
            >
              <v-checkbox
              ...
              <v-avatar
                color="primary-lighten-5"
                size="40"
                class="mx-3 text-primary font-weight-bold"
              >
        ...
        <v-card-actions class="pa-6 pt-0">
          ...
          >
            <v-icon start size="18">mdi-flash</v-icon> Generate
            {{ selectedEmps.length }} Payroll(s)
          </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
const payrollStats = computed(() => {
  const totalGross = payrollRecords.value.reduce(
    (sum, p) => sum + (p.baseSalary + p.overtime + p.bonuses),
    0,
  );
  const totalNet = payrollRecords.value.reduce(
    (sum, p) => sum + p.netSalary,
    0,
  );
  const paidCount = payrollRecords.value.filter(
    (p) => p.status === "Paid",
  ).length;
  ...
  return [
    ...
    {
      label: "Paid",
      value: `${paidCount} / ${payrollRecords.value.length}`,
...
const printPayslip = () => {
  ...
  printWindow.document.write(
    "<html><head><title>Aura HR - Payslip</title>" + css + "</head><body>",
  );
...
const employeeDetails = computed(() => {
  ...
  return {
    ...
    Department:
      activePayRecord.value.employeeObj?.department?.name || "Unknown",
...
const grossPay = computed(() => {
  ...
  return (
    activePayRecord.value.baseSalary +
    activePayRecord.value.overtime +
    activePayRecord.value.bonuses
  );
});
...
.payroll-input :deep(.v-field__input) {
  background-color: rgb(var(--v-theme-surface)) !important;
}

.v-theme--dark .payroll-input :deep(.v-field__input) {
  background-color: rgba(var(--v-theme-surface), 0.08) !important;
}
